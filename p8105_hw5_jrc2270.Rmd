...
## Problem 2
In this longitudinal data, the group receiving the experimental treatment seems to have increasing values over the study period, while the group control group has values that stay fairly constant. By week 8, there is a clear difference between the experimental group and the control group.
```{r, message=FALSE}
long <- 
  list.files("data") %>%
  tibble(file = .) %>%
...
## Problem 3
By running 5000 t-tests each on samples from populations with different means, we can see that as effect size increases, power quickly approaches 100%.
```{r}
...
The mean estimate across samples is, of course, similar to the true mean. For the samples where the null hypothesis was rejected, the mean estimate is close to the true mean for samples with a large effect size and for the sample under the null hypothesis. However, the mean estimate is slightly higher than the true mean for samples with a small effect size. This is because samples with estimates close to the null hypothesis have large p-values.
```{r}
power_by_mean_df %>%
  group_by(mu) %>%
...
We gain a little more insight by looking at the distribution of estimates.
```{r}
power_by_mean_df %>%
  group_by(mu) %>%
